...
int checkElem(float mass[], int size, int elem){
...
int inputOneTime(int inp_mass[], int out_mass[], int size){
    int qua=0;
    float tmp_mass[size];
    for(int i = 0; i < size; i++){
        tmp_mass[i] = -0.5; 
...
void inputCountsElems(int inp_mass[], int out_mass[], int size){
    
    for(int i=0;i<size;i++){
            
            out_mass[i]=countElem(inp_mass, size, inp_mass[i]);
            
    }
    
}

int quaOfDigits(int iks){
    int qua=0;
    while (iks>0){
        qua++;
        iks/=10;
...
void inputCiphers(int mass[],int size, int iks){
    for(int i=0;i<size;i++)
    {
        mass[size-i-1]=iks%10;
        iks/=10;
    }

}

int makeNumber(int mass[],int size)
{
    int num=0;
    for(int i=size-1;i>=0;i--)
        num=num*10+mass[i];
...
int main(){
    int N;
    scanf("%d",&N);
    int ciphers[quaOfDigits(N)];
    inputCiphers(ciphers,quaOfDigits(N), N);
    bubbleSort(ciphers,quaOfDigits(N));
    printf("%d", makeNumber(ciphers,quaOfDigits(N)));
    return 0;
}